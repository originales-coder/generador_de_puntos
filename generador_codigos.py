...
for jugadora in jugadoras:
  ...
  with c1:
    st.markdown(
        f"<div style='font-size: 8rem; font-weight: bold; color: #111; line-height: 1.1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{jugadora}</div>",
        unsafe_allow_html=True,
    )
  with c2:
    st.markdown(
        f"<div style='font-size: 8rem; font-weight: bold; color: #800020; line-height: 1.1; text-align: center;'>{puntos}</div>",
        unsafe_allow_html=True,
    )
  with c3:
